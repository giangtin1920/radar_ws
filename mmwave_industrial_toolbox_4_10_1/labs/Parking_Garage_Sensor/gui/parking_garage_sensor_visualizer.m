...
if(SETUP_VIA_GUI)
    ...
    cfgFile.path = '../chirp_configs/';
    cfgFile.name = 'parking_garage_sensor_68xx_ISK_getFlashRecord.cfg';
    logFile.path = '';
    logFile.name = 'parkingGarage.txt';
    offset.height = 2.5;
    offset.el = -30; 
    comPort.cfg = 11;
    comPort.data = 12;
    ...
    parking.display = 1;                        %enable parking spot detection mode, draw parking spot on graph, enable temporal filtering of static pts
    parking.useParkingConfigFile = 1;           %read in parking spot orientation data from file
        
    if parking.useParkingConfigFile
        parking.filePath = 'parkingSetup/';
        parking.fileName = 'parkingGarageSetup_4cars.csv';
        parking = parseParkingSetup(parking, parking.filePath, parking.fileName);
    else
        parking.constantFrameCountThresh = 3;       %number of frames in staticPtCloudHistoryThresh frames a static pt must be active before it is displayed
        parking.staticPtCloudHistoryThresh = 70;    %number of frames to look for constant pts
        parking.numSpots = 4;                       %total number of parking spots in the FOV, include a spotSize for each spot below.
        parking.parkingSpotSize = zeros(parking.numSpots,4);
        parking.hparkingSpotSize(1,:) = [2.2,2,1.8,4];      % location and size of parking spot. 
        parking.hparkingSpotSize(2,:) = [0.2,2,1.8,4];      % Measured from bottom left corner of spot (in m) [X,Y,width,height]
        parking.hparkingSpotSize(3,:) = [-1.8,2,1.8,4];
        parking.hparkingSpotSize(4,:) = [-3.8,2,1.8,4];
        parking.pSpotOccupiedPtsThresh = 2;         %number of static points required inside parking space before it is considered occupied
        parking.pSpotOccupiedHyst = 40;             %number of occupied frames before a parking spot is marked occupied
        parking.snrThresh = 15;                     %snr filter for static point cloud
    end
...
styleStaticPtCloud = {'LineStyle','none','Marker','square','Color','m','MarkerSize',20,'LineWidth',4};
hStaticPtCloud = line(hAx3D,1,1,1, styleStaticPtCloud{:});

% init handle for visualizing tracked objects
styleTrackObj = {'LineStyle','none','Marker','o','MarkerEdge','w','MarkerSize',40,'LineWidth',4};
...
% show parking spot indicator
if(parking.display)
    %hParkingSpot = struct;
    
    for i = 1:parking.numSpots
        hParkingSpot(i) = drawParkingSpot(hAx3D, parking.hparkingSpotSize(i,:));
    end
    
    if (parking.displayWarningSpot)
        hWarningSpot = drawWarningSpot(hAx3D, parking.hWarningSpotSize(1,:));
    end
        
    pSpotOccupied = zeros(parking.numSpots,1);
    constantOccupiedCount = zeros(parking.numSpots,1);
    constantWarningOccupiedCount = 0;
    
    staticPtCloudHistoryNum = 1;
    staticPtCloudHistory.XData = {};
    staticPtCloudHistory.YData = {};
    staticPtCloudHistory.ZData = {};
        
    constantFrameCount = 0;
    %constantOccupiedCount = 0;
        ...
        if(1)
            ...
            if(SHOW_STATIC_PT_CLOUD)            
                if(HAVE_VALID_STATIC_PT_CLOUD)
                    ...
                    snrData = newframe.staticSideInfo.snr;
                                
%                     if(staticPtCloudHistoryNum <= staticPtCloudHistoryThresh)
%                         staticPtCloudHistory.XData{1,staticPtCloudHistoryNum} = hStaticPtCloud.XData;
%                         staticPtCloudHistory.YData{1,staticPtCloudHistoryNum} = hStaticPtCloud.YData;
%                         staticPtCloudHistory.ZData{1,staticPtCloudHistoryNum} = hStaticPtCloud.ZData;
%                         staticPtCloudHistoryNum = staticPtCloudHistoryNum + 1;
%                     else
                    if (parking.display)
                        [hStaticPtCloud.XData,  hStaticPtCloud.YData, hStaticPtCloud.ZData, staticPtCloudHistory.XData, ...
                         staticPtCloudHistory.YData, staticPtCloudHistory.ZData] = filterStaticPointsforParking(...
                                     hStaticPtCloud.XData, hStaticPtCloud.YData, hStaticPtCloud.ZData, ...
                                     staticPtCloudHistory.XData, staticPtCloudHistory.YData, staticPtCloudHistory.ZData,...
                                     staticPtCloudHistoryNum, parking.constantFrameCountThresh,parking.staticPtCloudHistoryThresh,...
                                     snrData, parking.snrThresh);

                        if (staticPtCloudHistoryNum < parking.staticPtCloudHistoryThresh)
                            staticPtCloudHistoryNum = staticPtCloudHistoryNum + 1;   
                        end
                    end
            ...
            if (parking.display)
                for i = 1:parking.numSpots
                    pSpotOccupiedTemp = isParkingSpotOccupied(parking.hparkingSpotSize(i,:),hStaticPtCloud.XData,hStaticPtCloud.YData,hStaticPtCloud.ZData,parking.pSpotOccupiedPtsThresh);
                    if (pSpotOccupiedTemp && (constantOccupiedCount(i) <= parking.pSpotOccupiedHyst))
                        constantOccupiedCount(i) = constantOccupiedCount(i) + 1;
                    elseif (pSpotOccupiedTemp == 0 && (constantOccupiedCount(i) > 0))
                          constantOccupiedCount(i) = constantOccupiedCount(i) - 1;
                    end

                    %add histeresis on spot occupation
                    if (constantOccupiedCount(i) > parking.pSpotOccupiedHyst)
                        pSpotOccupied(i) = 1;
                    elseif (constantOccupiedCount(i) < 3)
                        pSpotOccupied(i) = 0;
                    end

                    %update parking spot color to indicate occupation
                    if (pSpotOccupied(i))
                        hParkingSpot(i).spot1.FaceColor = [0.4,0,0];
                        hParkingSpot(i).spot1.EdgeColor = [1,0,0];
                        
                        %image('CData',carTopIcon,...
                        %    'XData',[parking.hparkingSpotSize(i,1) parking.hparkingSpotSize(i,1)+parking.hparkingSpotSize(i,2)],...
                        %    'YData',[parking.hparkingSpotSize(i,3) parking.hparkingSpotSize(i,3)+parking.hparkingSpotSize(i,4)]);
                        
                    else
                        hParkingSpot(i).spot1.FaceColor = [0,0.4,0];
                        hParkingSpot(i).spot1.EdgeColor = [0,1,0];
                    end
                end
                
                %logic to determine if warning spot is occupied 
                if (parking.displayWarningSpot)
                    wSpotOccupiedTemp = isWarningSpotOccupied(parking.hWarningSpotSize(1,:),hStaticPtCloud.XData,...
                                           hStaticPtCloud.YData,hStaticPtCloud.ZData,parking.wSpotOccupiedPtsThresh);


                    if (wSpotOccupiedTemp && (constantWarningOccupiedCount <= parking.wSpotOccupiedHyst))
                            constantWarningOccupiedCount = constantWarningOccupiedCount + 1;
                    elseif (wSpotOccupiedTemp == 0 && (constantWarningOccupiedCount > 0))
                          constantWarningOccupiedCount = constantWarningOccupiedCount - 1;
                    end

                    %add histeresis on spot occupation
                    if (constantWarningOccupiedCount > parking.wSpotOccupiedHyst)
                        wSpotOccupied = 1;
                    elseif (constantWarningOccupiedCount < 3)
                        wSpotOccupied = 0;
                    end


                    if (wSpotOccupied)
                        hWarningSpot.spot1.FaceColor = [0.6,0,0];
                        hWarningSpot.spot1.EdgeColor = [1,0,0];
                    else
                        hWarningSpot.spot1.FaceColor = [0.2,0.2,0];
                        hWarningSpot.spot1.EdgeColor = [0.5,0.5,0];
                    end
